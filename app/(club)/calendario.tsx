...
const PickerBtn = ({
  ...
  return (
    ...
    >
      <Text
        style={{ color: value ? colors.texto : colors.subtexto, fontSize: 15 }}
      >
        {value || label}
      </Text>
    </TouchableOpacity>
...
export default function Calendario() {
  ...
  const [exportTypes, setExportTypes] = useState<("TRAINING" | "MATCH")[]>([
    "TRAINING",
    "MATCH",
  ]);
  ...
  const handleCreateEvent = async () => {
    setCreateError(""); // Limpiamos el error cada vez que pulsa el botón
    ...
    if (createType === "MATCH" && !form.opponentName?.trim()) {
      setCreateError(
        "El nombre del rival es obligatorio para crear un partido.",
      );
      return;
    }
    if (
    ...
    try {
      ...
      const candidatos: CalendarEvent[] = rows.map((row) => {
        ...
        return {
          ...
          type:
            row.type === "MATCH" || row.Tipo === "Partido"
              ? "MATCH"
              : "TRAINING",
          startTime,
          endTime,
        ...
        <View style={{ paddingHorizontal: 14 }}>
          ...
          <View style={styles.legendRow}>
            <View style={styles.legendItem}>
              ...
              <Text style={[styles.legendText, { color: c.subtexto }]}>
                Partido
              </Text>
            </View>
            <View style={styles.legendItem}>
          ...
          <View style={styles.centeredOverlay}>
            <View style={[styles.centeredBox, { backgroundColor: c.fondo }]}>
              ...
              <Text
                style={[
                  styles.metaText,
                  { color: c.subtexto, marginBottom: 14 },
                ]}
              >
        ...
        >
          ...
          >
            ...
            >
              ...
              <Text
                style={[styles.modalTitle, { color: c.texto, marginBottom: 4 }]}
              >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {item.type === "MATCH"
                                ? (MATCH_TYPE_DISPLAY[item.matchType ?? ""]
                                    ?.badge ?? "PARTIDO")
                                : "ENTRENO"}
                            </Text>
                          </View>
                ...
                <View style={{ flexDirection: "row", gap: 10, marginTop: 15 }}>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setCreateModal(false);
                      setEditingEvent(null);
                      setCreateError(""); // Limpiamos al cancelar
                    }}
                  >
                    <Text style={[styles.btnCrearText, { color: c.texto }]}>
                      Cancelar
          ...
          <DateTimePicker
            ...
            onChange={(_, selected) => {
              ...
              if (selected) {
                ...
                setForm((f: any) => ({ ...f, date: toDateString(selected) }));
              }
        ...
        {showTimePicker && (
          <DateTimePicker
            ...
            onChange={(_, selected) => {
              if (selected) {
                setPickerDate(selected);
                setForm((f: any) => ({ ...f, time: toTimeString(selected) }));
              }
          ...
          <DateTimePicker
            ...
            onChange={(_, selected) => {
              if (selected) {
                ...
                setForm((f: any) => ({
                  ...f,
                  recurringEndDate: toDateString(selected),
                }));
              }
